...
class Pawn {
    ...
    constructor(color, i, j) {
        this.color = color; // The color of the pawn, either black or white
        this.i = i; // The row position of the pawn on the chess board
        this.j = j; // The column position of the pawn on the chess board
    }

    // Calculate and return all possible moves for this pawn on the given Board
    getMoves(Board) {
        let coordinates = []; // Stores the list of coordinates to which the pawn can move
        // Check if it's our pawn's turn based on the color
        if (Board.Side == this.color) {
            ...
            if (this.i > 1) {
                ...
                if (Board.Squares[this.i - 1][this.j].piece == undefined) {
                    ...
                    // Check for initial double move if the pawn hasn't moved yet
            ...
            // Handling moves for the computer's pawn which moves in the opposite direction
            if (this.i < 8) {
                // Forward movement by one square
                if (Board.Squares[this.i + 1][this.j].piece == undefined) {
                    coordinates.push(new Coordinate(this.i + 1, this.j));
    ...
    // Determines potential check positions based on the pawn's current position and movement capabilities
